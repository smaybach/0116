...
import com.example.dingdan.utils.CustomJiaJian;
...
public class RecyAdapter extends RecyclerView.Adapter<RecyAdapter.MyViewHolder>{
     ...
     * 添加数据并更新显示
     * */
    public void add(GoodsBean cartBean){
        //传进来的是bean对象
        if(list == null){
            list = new ArrayList<>();
        }
        //第一层遍历商家和商品
        for (GoodsBean.DataBean shop : cartBean.getData()){
     ...
     * 设置数据源,控制是否显示商家
     * */
    private void setFirst(List<GoodsBean.DataBean.ListBean> list) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
       ...
       /* //拆分images字段
        ...
        holder.item_delete.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                retrofit2.Call<ResponseBody> call = RetrofitUtils.getInstance().delete(list.get(position).getPid());
     ...
     * 计算总价的方法
     * */
    private void sum(List<GoodsBean.DataBean.ListBean> list){
    ...
    public static class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public MyViewHolder(View itemView) {
            ...
            simpleDraweeView = (SimpleDraweeView) itemView.findViewById(R.id.item_face);
        }
    }
